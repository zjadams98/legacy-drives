...
def load_cache(cache_file: str = CACHE_FILE) -> Tuple[List[Opportunity], Set[str], int, List[LegacyDriveRow]]:
    """Load saved JSON cache if present (backward compatible with older keys)."""
    ...
    # Canonical key is legacydrive_rows; still read debug_rows for backward compatibility.
...
def seasons_to_load(last_season_processed: int, current_season: int) -> List[int]:
    """Only load seasons not already fully processed."""
    if last_season_processed < current_season - 1:
        return list(range(last_season_processed, current_season + 1))
    return [current_season]


def import_regularseason_pbp(seasons: Sequence[int]) -> pd.DataFrame:
    """Import regular season PBP for requested seasons."""
...
def build_drive_starts(pbp_period: pd.DataFrame) -> pd.DataFrame:
    """Get the first row of each drive (drive start)."""
    return (
...
def get_qb_for_drive(
    ...
    """Determine QB ID/name for a drive with robust fallbacks."""
    qb_id = None
    qb_name = None
...
def sort_legacydrive_rows(rows: List[LegacyDriveRow]) -> List[LegacyDriveRow]:
    """Stable chronological order for embedding/caching."""
    return sorted(
        rows,
        key=lambda r: (
...
def build_leaderboard_records(opportunities: Sequence[Opportunity], name_map: Dict[str, str]) -> pd.DataFrame:
    """Build the aggregated leaderboard table."""
...
def process_new_games(
    ...
    """Compute opportunities + per-drive rows for games not yet cached."""
    if not new_games:
        return
    ...
    for _, row in opps.iterrows():
        ...
        if period == "Q4":
            ...
            # Regular-season rule difference: pre-2012 FG on 1st OT drive is Success.
...
def main() -> None:
    ...
    pbp_all = import_regularseason_pbp(seasons)
    ...
    # Build name maps for display (use whatever we loaded, then merge into persistent map).
    passer_name_map: Dict[Any, str] = (
        pbp_all.groupby("passer_id")["passer"].first().to_dict() if not pbp_all.empty else {}
    )
    qb_name_map: Dict[Any, str] = {}
    if not pbp_all.empty and "qb_id" in pbp_all.columns and "qb" in pbp_all.columns:
    ...
    if not pbp_new.empty:
        ...
        if new_games:
            # Defensive: should not happen, but keep messaging consistent.
